...
uses
  ...
  StdCtrls, Graphics, MisUtils, alexiaLex, Globales, FormConfig, EditView,
  FrameLateralPanel, FrameEditView, FrameMessagesWin, FrameCfgExtTool, FrameFileExplor,
  //Adaptadores
  adapterBase, adapter6502, adapterEditor;
type
  { TfrmPrincipal }
  ...
  published  //Eventos del formulario
  ...
  public     //Adaptadores
    msgManager : TMessageManager;  //Gestor de mensajes
    currComp   : TAdapterBase;     //Compilador actual
    adapEditor : TAdapterEditor;   //Adaptador para editor normal
    adapter6502: TAdapter6502;     //Adaptador para compilado al 6502
  ...
  private    //Funciones de apoyo a las acciones
  ...
  published  //Acciones
    ...
    acViewMsgPan: TAction;    procedure acFilCloseFileExecute(Sender: TObject);
    procedure acFilOpenFolderExecute(Sender: TObject);
    ...
    procedure acFilCloseFolderExecute(Sender: TObject);
...
{ TfrmPrincipal }
{$region "Controles"}
//Eventos que se asignan en tiempo de diseño y que deben estar como "Published"
...
//  fraCentPanel.AddPage(fraEditView1, 'editor', 'Editor', '');
  ...
  Config.OnPropertiesChanges := @Config_Changed;
  ...
  Config_Changed;
  //Carga últimos archivos abiertos y activa el que estaba activo.
  if Config.LoadLast then begin
     fraEditView1.LoadListFiles(Config.filesClosed);
     fraEditView1.SelectEditor(Config.fileActive);
  end;
  Timer1.Enabled := true;
end;
procedure TfrmPrincipal.FormCloseQuery(Sender: TObject; var CanClose: boolean);
var
  lstClosedFiles, curFile: string;
begin
  if fraEditView1.CloseAll(lstClosedFiles, curFile) then begin
     CanClose := false;   //cancela
  end else begin
    //Se va a cerrar. Guarda la lista de archivos que estaban abiertos y el archivo actual.
    Config.filesClosed := lstClosedFiles;
    Config.fileActive := curFile;
  end;
end;
...
{$EndRegion}
//Configuración
procedure TfrmPrincipal.Config_Changed;
...
begin
  case msginfo.row of   //Aquí vienen los códigos de mensajes
...
{$EndRegion}
{$region "Funciones de apoyo a las acciones'"}
...
procedure TfrmPrincipal.acFilOpenFolderExecute(Sender: TObject);
{Abre un folder.}
begin
