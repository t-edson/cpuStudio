...
//    function IsDrive: boolean;  //Indica si el nodo es una unidad
  ...
  TEvFileOperation= procedure(file0: string) of object;
  TEvFileToClose  = procedure(file0: string; var Cancel: boolean) of object;
  TEvFileRenamed  = procedure(oldFile, newFile: string) of object;
  ...
  published
    ...
    PopupOther: TPopupMenu;
    TreeView1: TTreeView;
    procedure lblOpenFolderMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure lblOpenFolderMouseEnter(Sender: TObject);
    procedure lblOpenFolderMouseLeave(Sender: TObject);
  ...
  private
    FBackColor: TColor;
    FTextColor: TColor;
    OldNodeName: String;    //nombre actual de nodo en edición
    procedure ActualPanelArc;
    procedure ExpandNodeFile(Node: TTreeNode; expan: Boolean);
    procedure LeeFilt(lfil: TStringList);
    procedure ReadDirectory(Item0: TTreeNode; filtro: TStringList;
  ...
  public  //Eventos
    ...
    OnRightClickFolder: TEvClickOnFile;   //Click derecho en carpeta
    OnKeyEnterOnFile  : TEvClickOnFile;   //Se presionó la tecla enter en un archivo
    OnCloseFile       : TEvFileToClose;   //Se pide cerrar un archivo
    OnRenamedFile     : TEvFileRenamed;   //Se indica que ha habido un cambio de nombre
    OnOpenFolder    : procedure(Sender: TObject) of object;  //Se pide abrir un folder
    OnCloseFolder   : procedure(Sender: TObject) of object;  //Se pide cerrar un folder
  public
    NewFileName  : string;       //nombre por defecto de archivo nuevo
    NewFolderName: string;       //nombre pord efecto de carpeta nueva
    InternalPopupFolder: boolean; //Permite activar el menú contextual interno
    InternalPopupFile  : boolean; //Permite activar el menú contextual interno
    curNod       : TExplorNode;   //Nodo actual. Se usa como variable temporal
    property TextColor: TColor read FTextColor write SetTextColor;
    property BackColor: TColor read FBackColor write SetBackColor;
    procedure OpenFolder(currPath: string);
...
var
  nod: TTreeNode;
begin
  if TreeView1.Items.Count = 0 then exit;  //Sin carpeta abierta.
  if ExtractFilePath(arch8) = '' then begin
   ...
   while nod <> nil do begin
     if LocateInNode(TExplorNode(nod), arch8) then exit;
...
procedure TfraFileExplor.ExpandNodeFile(Node: TTreeNode; expan: Boolean);
...
begin
   ...
   ReadDirectory(Node, filtros, expan);
   filtros.Free;
end;
procedure TfraFileExplor.ReadDirectory(Item0: TTreeNode; filtro: TStringList;
                                       expandir: boolean = false);
...
begin
  ...
  for nod in TreeView1.Items do
    if nod.Visible and nod.Expanded then begin
      //mantiene expansión
//      ReadDirectory(nod, filtros, true);
...
//    if nod = nil then ShowMessage('nil') else ShowMessage(nod.Text);
    ...
    ReadDirectory(nodEx, filtros, true);
  end;
  nodExpan.Free;
  filtros.Free;
end;
...
procedure TfraFileExplor.OpenFolder(currPath: string);
...
procedure TfraFileExplor.CloseFolder;
begin
  TreeView1.Items.Clear;
  TreeView1.Visible := false;
end;
//Eventos de lblOpen
procedure TfraFileExplor.lblOpenFolderMouseEnter(Sender: TObject);
begin
  lblOpenFolder.Font.Bold := true;
end;
procedure TfraFileExplor.lblOpenFolderMouseLeave(Sender: TObject);
begin
  lblOpenFolder.Font.Bold := false;
end;
procedure TfraFileExplor.lblOpenFolderMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  mnOpenFolderClick(nil);
end;
...
//  if (node.Count = 1) and (node.Items[0].Text = ' ') then begin
    ExpandNodeFile(TExplorNode(Node), false);
//  end;
end;
...
var
  NewNodeName, tmpTxtPath, tmpText: String;
  nodEx: TExplorNode;
begin
  tmpTxtPath := TExplorNode(Node).txtPath;   //Guarda por si se necesita.
  tmpText    := Node.Text;                   //Guarda por si se necesita.
  TExplorNode(Node).txtPath := Node.Text;  //Actualiza "txtPath" porque de aquí sale el nombre.
  NewNodeName := TExplorNode(Node).GetPath;
  if OldNodeName = NewNodeName then exit;  //Mismo nombre
  if FileExists(NewNodeName) then begin
    MsgExc(TXT_ALREAD_EXIST);
    TExplorNode(Node).txtPath := tmpTxtPath;   //Retorna al nombre anterior
    Node.Text := tmpText;
    exit;
  end;
  //Hay que renombrar
  try
    RenameFile(OldNodeName, NewNodeName);
    if OnRenamedFile<>nil then OnRenamedFile(OldNodeName, NewNodeName);
  except
    MsgErr(TXT_ERR_RENAMING);
  end;
  //Refresca
  if Node.Parent<> nil then begin
    if node.Parent.Expanded then ExpandNodeFile(node.parent, true);   //refresca
  end;
  //Selecciona el nodo
//  nodEx := nodRuta(NewNodeName);  //toma a partir de su ruta, porque al refrescar se pierde la ref.
...
begin
  ...
  if (Shift = [ssCtrl]) and (Key = VK_F4) then begin
    if nodEx = nil then exit;     //verifica
    if OnCloseFile<>nil then OnCloseFile(nodEx.GetPath, Cancel);
  end;
...
var
  nodEx: TExplorNode;
begin
  //Ve si se ha hecho click fuera de algún nodo para quitar la selección.
  if not Assigned(TreeView1.GetNodeAt(X, Y)) then begin
    TreeView1.Selected := nil;       //Quita la selección
  end;
  //Procesa la acción
  if Button = mbRight then begin
    //se soltó click derecho
    nodEx := SelectedNode;   //lee seleccionado
    if nodEx = nil then begin
      PopupOther.PopUp;
      exit;     //verifica
    end;
    //procesa eventos
...
begin
  ...
  if curNod.Expanded then ExpandNodeFile(curNod, true);   //refresca
...
begin
  ...
  OldNodeName := curNod.GetPath; //Guarda nombre actual del nodo
end;
...
begin
  ...
  try
    fileDel := UTF8ToSys(curNod.GetPath);
    if FileExists(fileDel) then begin
      //Close if file is opened
      Cancel := false;
      if OnCloseFile<>nil then OnCloseFile(curNod.GetPath, Cancel);
      if Cancel then exit;
      //Delete from disk
      //DeleteToBin(PChar(fileDel));  //envía a papelera
      DeleteFile(fileDel);
    end;
  finally
...
begin
  if OnOpenFolder<>nil then begin
    OnOpenFolder(nil);
  end;
end;
procedure TfraFileExplor.mnCloseFolderClick(Sender: TObject);
begin
